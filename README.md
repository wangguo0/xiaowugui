...
WebHomeTV 是基于 FongMi / CatVod 生态二次开发的 Android 影音应用,保留原有点播、直播、Spider、解析、投屏、本地 HTTP 服务等能力,并重点增强了 **WebHome 自定义首页**、**App Native SDK**、**管理页面**、**WebHome 扩展**、**登录态学习/同步**、**网盘链接检测**、**站点健康排序**、**播放记录开放与上报** 和 **Nostr/TMDB 推荐首页**。
...
- **播放记录上报**:用户本地配置 Webhook 端点,App 按播放进度、完播等时机上报安全字段;爬虫可通过 `/api/playback/current` HTTP API 读取当前播放记录。
