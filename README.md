...
首次构建或清理依赖后，先生成本地 Media3 Maven 产物：

```bash
bash scripts/build_media_deps.sh --clean
```

当前播放层依赖 FongMi 定制 Media3：

- `FongMi/media` 使用 `release-1.10.1-fongmi`，版本锁定在 `third_party/media-lock.json`。
- 脚本会把 `androidx.media3:*:1.10.1-fongmi` 发布到 `third_party/maven`。
- `third_party/maven` 和 `third_party/sources` 是本地生成目录，不提交仓库。
- `settings.gradle` 会优先读取 `third_party/maven`，保证 `MediaTitle`、弹幕 UI 等 FongMi 扩展 API 可用。
- `nextlib-media3ext` 使用 `io.github.anilbeesetti:nextlib-media3ext:1.10.0-0.12.1`，提供 FFmpeg renderer。

如果构建时报找不到 `androidx.media3:*:1.10.1-fongmi`，先重新执行 `bash scripts/build_media_deps.sh --clean`。
