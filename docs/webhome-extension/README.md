...
- 调用 `fm.play()` 播放直链；`fm.vodInline()` 构造多集播放并按集回调解析；需要播放页图片首帧更稳时，在详情页阶段调用 `fm.preloadArtwork(pic, wallPic)` 后台预热。
...
进入 App 推送播放链路（`SiteApi.PUSH`）。传了 `headers` 或 `credentials: "include"` 时，播放 URL 自动转成 `/webResource` 网关地址。`url` 也可以是 App 可识别的特殊协议地址（主文档 26.12）。`options.pic` 是海报/默认 artwork，`options.wallPic` 是播放页背景图。

#### `fm.vod(siteKey, vodId, title, pic, options)` — 打开 CSP 站点影片

进入对应站点的原生详情/播放链路，`vodId` 即该站 Spider 的 `vod_id`。`pic` 是海报，`options.wallPic` 是播放页背景图。

#### `fm.vodInline(payload)` — 临时多集 / 按集解析

见第 8 章专述。

#### `fm.preloadArtwork(pic, wallPic)` — 后台预热播放页图片

把海报和播放页背景图提交给原生 Glide 预热，立即返回，不等待下载完成，也不阻塞之后的播放跳转。扩展在详情页阶段拿到图片后即可调用；用户点击"App播放"时应直接调用 `fm.play` / `fm.vod` / `fm.vodInline`，不要点击后再等待预热。

图片语义：`pic` 用于竖版海报/播放器默认 artwork；`wallPic` 用于播放页背景图，推荐传横屏剧照/backdrop。App 不会自动判断横竖屏；没有 `wallPic` 时播放页背景会用 `pic` 兜底。
...
`vod_pic` / `pic` 是竖版海报；`wallPic` 是播放页背景图，推荐传横屏剧照。没有 `wallPic` 时，播放页背景会用海报兜底。
