...
当前项目已经提交完整构建所需的本地依赖，clone 后不需要额外准备 AAR 或重新生成 Media3 产物：
...
仓库已经包含 `third_party/maven`，clone 后可直接构建。`scripts/build_media_deps.sh --clean` 只在升级或重新生成 Media3 产物时需要执行。

`third_party/media-lock.json` 记录 Media3 锁定版本。`settings.gradle` 已配置优先读取本地 Maven 仓库，`app/build.gradle` 会强制所有 `androidx.media3` 依赖统一使用 `1.10.1-fongmi`。
...
`scripts/build_media_deps.sh --with-nextlib` 可以按锁定版本准备 NextLib 源码，方便审查或后续本地化构建。完整本地构建 NextLib 需要 NDK、CMake 和 FFmpeg 环境，默认构建链路不要求开发者本地发布 NextLib AAR。
