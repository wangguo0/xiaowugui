...
| 格式 | 示例 | 说明 |
| --- | --- | --- |
| 默认代理地址 | `socks5://192.168.1.10:1080` | 规则为空时，对所有 host 使用默认代理 |
| 文本规则 | `*.example.com socks5://192.168.1.10:1080` | 每行一条，前半段是 host/正则列表，后半段是代理地址；只写代理地址时等价于 host 为 `*` |
| JSON raw | 见下方示例 | 与配置文件 `proxy` 结构一致，适合从文档或配置里复制 |
  ...
  "proxy": [
    {
      ...
      "urls": ["socks5://192.168.1.10:1080"]
    },
    {
      "hosts": ["*"],
      "urls": ["http://192.168.1.10:7890"]
    }
  ]
}
```
