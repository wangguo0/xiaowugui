...
| `immersive` | 隐藏原生 UI 和系统栏，显示原生恢复入口；返回键第一下恢复上一个 chrome 模式 | TV 系统栏长期隐藏，映射为 `tv-full` |
...
| `tv-overlay` | 不适用 | WebView 全屏铺底，顶部 toolbar 作为覆盖层显示 |
| `tv-full` | 不适用 | WebView 全屏 overlay，隐藏顶部 toolbar，由 WebHome 接管首屏 |

TV 端在 `tv-full` / `tv-toolbar-hidden` 首页边界按返回键时，会先临时切到 `tv-normal` 露出顶部 toolbar，并让 WebView 避开 toolbar；WebHome 内的“关闭首页全屏”也应使用 `tv-normal`，不能用会覆盖 WebHome 顶部内容的 `tv-overlay`。
...
- 详情页需要专注模式时优先调用 `fm.ui.setChrome({ mode: "immersive", restoreAffordance: "native" })`，关闭详情或返回主页时 `fm.ui.restoreChrome()` 或 `fm.ui.setChrome({ mode: "edge" })` 恢复首页融合。TV 端 `edge` / `immersive` 会映射到 `tv-full`；`fm.ui.setToolbar(false)` 仅作为旧脚本兼容，TV 端会切到 `tv-toolbar-hidden`。
