...
### 3.1 Media3 和 NextLib 依赖准备

当前项目使用 FongMi 定制 Media3，而不是 Maven Central 的官方 `androidx.media3`：

- `FongMi/media` 分支：`release-1.10.1-fongmi`。
- Media3 版本：`1.10.1-fongmi`。
- 版本锁：`third_party/media-lock.json`。
- 生成产物目录：`third_party/maven`。
- 源码缓存目录：`third_party/sources`。

首次构建、清理仓库或更新锁定版本后，先执行：

```bash
bash scripts/build_media_deps.sh --clean
```

这个脚本会按 `third_party/media-lock.json` 拉取 `FongMi/media`，把 App 需要的 Media3 模块发布到本地 Maven 仓库 `third_party/maven`。`settings.gradle` 已配置优先读取该本地仓库，`app/build.gradle` 会强制所有 `androidx.media3` 依赖统一使用 `1.10.1-fongmi`。

需要这套本地 Media3 的原因：当前 App 使用了 FongMi Media3 扩展 API，包括：
...
`scripts/build_media_deps.sh --with-nextlib` 可以按锁定版本准备 `FongMi/nextlib` 源码，方便审查或后续本地化构建。完整本地构建 NextLib 需要 NDK、CMake 和 FFmpeg 环境，默认构建链路不要求开发者本地发布 NextLib AAR。
