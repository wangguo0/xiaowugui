...
- 增强功能页集中放置“网盘检测”“站点健康排序”“管理页面”“站点注入”“Proxy 壳代理”“一键同步”“调试日志”等能力。
...
- “站点注入”会在当前点播配置加载完成后，把用户维护的 WebHome 或通用 CSP 站点插入 `sites` 列表。它有全局启用开关，也有每条站点的启用开关。
...
4. WebHome 类型只需要维护名称、Key、WebHome 地址和可选 Site JSON；适合把本地 HTML 文件、手写 HTML 代码或在线链接作为网页首页。新增条目的默认 Key 会根据名称生成稳定值，便于 WebHome 扩展使用 `cspKeyRegex` 匹配；手动修改 Key 后会保留用户填写值。
5. 通用 CSP 类型会显示 `type`、`api`、`ext`、`jar`、`click`、`playUrl`、`hide`、`searchable`、`changeable`、`quickSearch`、`indexs`、`timeout`、`categories`、`header`、`style` 等站点参数，也支持直接编辑完整 Site JSON。
6. 直播类型只生成注入到 `lives` 列表的 Live 条目，不额外创建独立直播接口或独立 CSP 加载方式。常用参数优先显示 `name`、`url`、`ua`、`epg`、`logo`，其中 `ua` 新增时默认 `okhttp`；`type` 和 `playerType` 不在界面展示，保存时默认写入 `type: 0`、`playerType: 2`。`header`、`timeout`、`groups`、`catchup`、`core`、`boot`、`pass` 等低频参数收在高级区，也支持直接编辑完整 Live JSON。
7. JSON raw 支持完整 registry、`sites`/`lives` 数组、单个 site object 或单个 live object；单个 live object 保存后会作为 `lives` 注入项处理。保存后会自动刷新当前点播配置和直播配置，让注入结果立即生效。
8. 站点注入文件存放在 `/sdcard/TV/CustomCsp/`。默认一键同步目录包含 `TV`，因此开启“Jar/脚本保存数据”并保留 `TV` 目录时会同步这些注入文件；如果用户从同步目录中移除了 `TV`，则不会自动同步。
