...
### 13.4 `/api/playback/current`

当前播放记录只读 API，供 JS/Python/PHP/Jar 等爬虫通过 HTTP 调用。接口只返回当前播放条目，不返回全局历史。调用方必须传入当前站点 `siteKey`，App 会校验它和当前播放记录一致。
...
错误返回 JSON：`400` 缺少 `siteKey`，`404` 当前无可读记录或站点不匹配，`405` 方法不支持。字段范围、示例和 Webhook 上报格式见 `docs/播放记录开放与上报方案.md`。

### 13.4.1 观影进度写入 API

观影记录同步提供本机修改 API，供 JS/Python/PHP/Jar 等爬虫或局域网工具把远端进度写回 App 本地历史，或清理误同步、已失效和测试写入的记录。调用前需要用户在“增强功能 -> 观影记录同步”中开启总开关和“允许本机 API 修改”；关闭修改开关时，`/api/playback/current` 仍可读取当前播放记录。
...
最小必填字段为 `siteKey/vodId/vodName/episodeName/positionMs/durationMs`；`updatedAt` 缺省时由 App 使用当前时间。批量写入支持 `{ "items": [...] }`，也兼容 `{ "records": [...] }`、`{ "data": [...] }`、`{ "list": [...] }` 或直接数组。合并时先用 `configKey` 映射本机点播接口并确定 `cid`，未提供 `configKey` 时写入当前点播接口；映射不到时跳过，避免不同接口的同名站点记录互相覆盖。主定位使用 `siteKey + vodId`，剧集优先比较 `episodeUrl`，其次 `flag + episodeName`，最后 `episodeName`；远端 `updatedAt` 新于本地 `History.createTime` 时才覆盖进度。可用 `docs/playback-progress-mock.json` 作为批量写入和远端同步的 mock 数据。

清理请求同样使用 JSON，支持单对象、直接数组或 `{ "items": [...] }`：
...
| `configKey` | 可选；优先用它定位本机点播接口空间。未提供时使用当前接口或本机 `cid` |
| `scope=all` + `confirm=true` | 清理当前接口下的全部观影历史；可传 `configKey` 或本机 `cid` 指定配置空间 |

单条响应为 `PlaybackProgressApplyResult` JSON，核心字段如下：

| 字段 | 说明 |
| --- | --- |
| `success` | 是否处理成功 |
| `action` | `created`、`updated`、`deleted`、`skipped` 或 `failed` |
| `message` | 跳过或失败原因 |
| `historyKey` | 写入或匹配到的本地历史 key |
| `configKey/siteKey/vodId/episodeName` | 调用方提交的核心定位字段 |
| `affected` | 写入为 1；清理时为实际删除条数 |
| `localUpdatedAt/remoteUpdatedAt` | 本地与远端更新时间 |

批量响应为 `PlaybackProgressBatchResult` JSON，包含 `total/applied/created/updated/deleted/skipped/failed/items`。单条失败不影响其它条目。
...
- “观影记录同步”当前用于管理用户本地 Webhook 端点和当前播放只读 API。目标界面分为“开关”“远端同步”“Webhook 上报”三块：顶层只显示总开关、本机 API 修改开关和状态摘要；远端同步源列表与 Webhook 端点列表点击后进入二级界面，新增/编辑都在独立表单中完成。Webhook 上报时机固定为进度更新、退出/暂停/切集前最终进度和自然完播，不再让用户选择；字段预设为“基础/标准/完整/自定义”，自定义字段使用带说明的多选列表。Webhook 和远端同步 token 均由用户服务端统一提供，App 只负责保存并随请求发送。远端同步源和本机修改 API 规范详见 `docs/播放记录开放与上报方案.md`。
...
该接口供 JS/Python/PHP/Jar 等爬虫读取当前播放记录。成功返回 `schema/timestamp/sessionId/dedupeKey/siteKey/vodId/vodName/episodeName/state/positionMs/durationMs/progress/completed` 等安全字段；缺少 `siteKey` 返回 400，当前无记录或站点不匹配返回 404。完整字段说明见 `docs/播放记录开放与上报方案.md`。
