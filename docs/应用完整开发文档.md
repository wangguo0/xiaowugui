...
- 增强功能：集中放置网盘检测、管理页面、站点注入、壳代理、一键同步、调试日志等能力。
...
- 站点注入：启用/禁用全局注入和单条站点，新增 WebHome 或通用 CSP，支持文件、代码、链接和 JSON 配置。
...
- 增强功能页集中放置“网盘检测”“管理页面”“站点注入”“Proxy 壳代理”“一键同步”“调试日志”等能力。
...
3. 规则为空：所有非本机目标 host 走默认代理。
4. 只代理指定域名：在“文本”里填 JSON raw，格式如下。
...
2. 全局开关关闭时，不会修改当前接口；单条站点关闭时，该条不会插入。
3. 插入位置限制在 `sites` 前 10 位内。界面显示为 1 到 10，显示“1”表示插入到 `sites[0]` 前面。
...
6. JSON raw 支持完整 registry、`sites` 数组或单个 site object。保存后会自动刷新当前点播配置，让注入结果立即生效。
7. 站点注入文件存放在 `/sdcard/TV/CustomCsp/`。默认一键同步目录包含 `TV`，因此开启“Jar/脚本保存数据”并保留 `TV` 目录时会同步这些注入文件；如果用户从同步目录中移除了 `TV`，则不会自动同步。
