...
| `/api/playback/progress` | POST/OPTIONS | 规划接口：写入单条观影进度，当前版本尚未实现 |
| `/api/playback/progress/batch` | POST/OPTIONS | 规划接口：批量写入观影进度，当前版本尚未实现 |
...
### 13.4.1 观影进度写入 API（规划）

观影记录同步的目标态会增加本机写入 API，供 JS/Python/PHP/Jar 等爬虫或局域网工具把远端进度写回 App 本地历史。当前版本尚未实现这些 endpoint，文档先固定字段和冲突规则，避免不同调用方各自定义格式。
...
写入能力应受“观影记录同步 -> 允许本机 API 写入”开关控制，默认关闭。用户配置 token 后，调用方通过 `Authorization: Bearer <token>`、`X-WebHTV-Token: <token>` 或 query `token=<token>` 传入，token 只用于鉴权，不参与主键、去重或合并计算。
...
最小必填字段为 `siteKey/vodId/vodName/episodeName/positionMs/durationMs/updatedAt`。批量写入使用 `{ "items": [...] }`，单批建议不超过 100 条。合并时主定位使用 `siteKey + vodId`，剧集优先比较 `episodeUrl`，其次 `flag + episodeName`，最后 `episodeName`；远端 `updatedAt` 新于本地 `History.createTime` 时才覆盖进度。完整接口、响应格式、校验规则和远端同步源规范见 `docs/播放记录开放与上报方案.md`。
...
- “观影记录同步”当前用于管理用户本地 Webhook 端点和当前播放只读 API。目标界面分为“开关”“远端同步”“Webhook 上报”三块：顶层只显示总开关、本机 API 写入开关、token 和状态摘要；远端同步源列表与 Webhook 端点列表点击后进入二级界面，新增/编辑都在独立表单中完成。Webhook 上报时机固定为进度更新、退出/暂停/切集前最终进度和自然完播，不再让用户选择；字段预设为“基础/标准/完整/自定义”，自定义字段使用带说明的多选列表。远端同步源和写入 API 规范详见 `docs/播放记录开放与上报方案.md`。
...
目标态用于让爬虫或局域网工具写入单条/批量观影进度，字段以 `siteKey/vodId/episodeName/positionMs/durationMs/updatedAt` 为核心。当前代码尚未实现写入 endpoint，不能直接调用。
