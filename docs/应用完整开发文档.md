...
- 增强功能：集中放置网盘检测、壳代理、一键同步、调试日志等能力。
...
| `paths` | `"TV\nTVBox\nTVData"` | 同步目录列表，每行一个外置存储相对路径；支持用户在“一键同步 -> 同步目录”里编辑 |
...
| 自定义目录 | 在一键同步界面点击“同步目录”编辑，每行一个目录；可填写 `TVBox`、`TVData/cookie`，也可填写 `/sdcard/TVBox`，内部会转成外置存储相对路径 |
...
`fm.ui.setToolbar(false)` 可请求隐藏原生 WebHome 顶部工具栏，`fm.ui.setToolbar(true)` 恢复显示。该能力主要用于 TV 端详情页等需要把空间留给剧照或内容的场景，手机端通常保持原生顶部操作区。
...
- 增强功能页集中放置“网盘检测”“Proxy 壳代理”“一键同步”“调试日志”等能力。
...
- “一键同步”用于局域网设备发现、推送/拉取同步、选择同步内容和同步目录，适合迁移接口、Jar 配置中心保存数据、WebHome 缓存、搜索记录、观看历史、收藏和设置。
...
| 格式 | 示例 | 说明 |
| --- | --- | --- |
| 默认代理地址 | `socks5://127.0.0.1:7897` | 规则为空时，对所有非本机目标 host 使用默认代理；适合本机 Clash、sing-box 等客户端 |
| 文本规则 | `example.com socks5://127.0.0.1:7897` | 每行一条，前半段是 host/正则列表，后半段是代理地址；只写代理地址时等价于 host 为 `*` |
| JSON raw | 见下方示例 | 与配置文件 `proxy` 结构一致；壳代理里单条规则省略或留空 `urls` 时，会使用上方默认代理地址 |
...
说明：`127.0.0.1` 是当前设备本机。如果 App 和 Clash/sing-box 装在同一台设备上，可填写 `socks5://127.0.0.1:7897`；如果代理客户端在另一台设备上，要填写那台设备的局域网 IP。App 会跳过目标地址为 `127.0.0.1` 或 `localhost` 的本地请求，避免文件服务、WebHome 网关等本地能力被代理，但这不影响使用 `127.0.0.1` 作为代理服务器地址。

调试日志开启后，壳代理会输出这些关键日志：`app proxy enabled/disabled` 表示开关和规则应用状态，`selector add/remove` 表示代理规则进入或移出 OkHttp 选择器，`select hit` 表示某个请求命中了壳代理规则，`select fallback reason=no-match/local-target/no-rule` 表示没有走壳代理及原因，`connectFailed` 表示代理连接失败。
...
- 实时拉取：`/debug/stream`，返回 `{ enabled, size, text }`，日志页默认每秒轮询一次。
...
- 覆盖范围包括 WebHome SDK 调用、WebView console、JS 异常、WebView provider 版本、WebView 主框架/资源加载错误、RenderProcess 崩溃、`fm.req`、资源网关、`pan.check`、`pan.play`、本地 HTTP 服务、全局 OkHttp 请求响应、SpiderDebug、push/pvideo 和播放器状态。
