...
日期：2026-06-10（2026-06-11 评审修订：经代码逐项核对与外部资料复核后，新增 TD-12/TD-13、Q14/Q15，补充 chrome 状态重建恢复、IME 避让路径、刘海区延伸保障与验收矩阵）

本文只做可行性和方案评估，不代表已实现。目标是解决手机端 App 首页顶部状态栏、底部导航栏/手势区与 WebHome 页面背景割裂的问题，并避免全屏后 WebHome 顶部/底部按钮被系统区域遮挡或失去恢复入口。
...
## 15. 对现有 WebHome 页面的影响评估（以 `demo/nostr.html` 为样本）

`demo/nostr.html`（约 1.35 万行）是一个完整的真实 WebHome 页面，能代表存量页面的写法。逐项核对后结论如下。
...
当前 `demo/` 目录只保留 `demo/nostr.html`。后续如新增 WebHome 示例页面，迁移时也必须按 B1 排查安全区写法，不要只改当前样本页。

## 16. 对《应用完整开发文档》的影响清单（暂不修改，实施时参考）

本方案落地后，`docs/应用完整开发文档.md` 需要同步更新以下位置（当前**先不动**，仅登记）。

| 位置 | 现状 | 实施后需要补/改 |
| --- | --- | --- |
| §5 点播站点配置（`homePage` / `webHome` 字段附近） | 无 WebHome chrome 默认模式字段 | 新增站点级 `chromeMode` 或 `webHomeChrome` 配置说明，用于 Native 在 WebView 首帧前预应用 `normal/edge/immersive/tv-*`；明确 HTML 内 `setChrome()` 只是运行时切换，不能作为默认 edge 的唯一机制 |
| §16 SDK 总览（`fm`/`fongmi` 映射表，约 `:1271-1292`） | 无 chrome/viewport 相关条目 | 新增 `fongmi.ui.setChrome`（`fm.ui.setChrome`）、`fongmi.ui.restoreChrome`（`fm.ui.restoreChrome`）、`fongmi.ui.getViewport`（`fm.ui.getViewport`）、`fongmi.app.openVod`（`fm.openVod`）；TV 端可选 `fongmi.app.openPush`、`fongmi.site.select/next/prev`、`fongmi.config.reload`（短别名按 §6.0：`fm.selectSite`/`fm.reloadConfig` 等，避开 `fm.reload`） |
| §16 `fm.ui.setToolbar` 语义说明（约 `:1287,:1296`） | 描述 setToolbar(false) 进入手机端沉浸 | 标注为 **legacy chrome API**，指向新的 `setChrome`；说明 setToolbar 映射到哪个 mode（按 Q1 决议） |
| 缺少「Chrome 模式」章节 | 无 | 新增章节：`normal/edge/immersive`（及 TV 的 `tv-normal/tv-overlay/tv-full`）定义、`setChrome` 参数（mode/statusBarStyle/navigationBarStyle/scrim/restoreAffordance）、状态机与恢复优先级 |
| 缺少「视口/安全区」章节（`fmviewport`、`--fm-*`） | 全文未记录 `fmviewport`、`--fm-web-*`、`--fm-safe-*` | 新增章节，列全 `fmviewport v2` 字段与全部 `--fm-*` CSS 变量；**重点写明 `--fm-safe-*` 与 `env()` 必须用 `max()` 去重、不能相加**（即 §15 B1 的踩坑），并给 `viewport-fit=cover` 要求 |
| §20.1 设备信息 `fm.device()`（`:1648`） | 返回 `/device` 字段，无安全区 | 若最终安全区只走 `getViewport`/`fmviewport`，需说明"安全区不在 `device()` 里，请用 `fm.ui.getViewport()`"，避免开发者误找 |
| §20.2/§20.3（`fm.site()`/`fm.config()`，`:1675/:1691`） | 是函数型 getter | 若新增 `fongmi.site.select/next/prev`、`fongmi.config.reload`，需在这两节补充：它们是同组的**额外方法**，短别名不能写成 `fm.site.x`/`fm.config.reload`（见 §6.0 冲突说明） |
| 命名规范说明（§16 开头，`:1219`） | 仅说"fongmi 全名、fm 短别名" | 补充 §6.0 的两条硬规则：app/navigation 在短别名扁平、`fm.device/site/config` 是函数不可挂子方法；并加"复用优先、禁止重复定义" |
| 迁移/兼容说明 | 无 | 新增"存量 WebHome 迁移指引"：§15 的 B1/R1/R2 改造清单，作为所有页面升级 edge 的统一步骤 |
| §11/全屏相关（如有 `Util.hideSystemUI` 行为描述） | 可能描述旧全屏行为 | 若 TD-2 移除 `FLAG_FULLSCREEN`、TD-8 改 TV 布局，相关行为描述需同步 |

> 说明：以上仅为登记，等方案真正实施、API 定稿后再回写《应用完整开发文档》，避免文档先于实现产生偏差。回写时以本方案 §6.0 命名规范和 §15 安全区去重约定为准。
