...
| minSdk | `24` |
| targetSdk | `28` |
| Android NDK | `28.2.13676358`，修改 MPV JNI/native 后重建 `libplayer.so` 使用 |
...
| MPV native/JNI | `third_party/mpv-player-jni` 保存 JNI 源码，`app/src/*/assets/mpv-libs/*` 保存随 APK 打包的 MPV native 库 |
...
- Android NDK 28.2.13676358。普通 Gradle 打包会使用仓库内置 `libplayer.so`，不需要每次重编；修改 MPV JNI/native 以后必须用该 NDK 重建。
...
- `app/src/arm64_v8a/assets/mpv-libs/arm64-v8a/` 或 `app/src/armeabi_v7a/assets/mpv-libs/armeabi-v7a/` 下的 MPV native 库
...
JNI 链接的是当前项目随 APK 打包的 MPV 库名，例如 `libmpv.so`、`libmvcodec.so`、`libmwscale.so`、`libc++_shared.so`。不要按通用 FFmpeg 名称改成 `libavcodec.so`、`libswscale.so`，否则会出现设备端 `dlopen failed`。
...
| 设备端报 `dlopen failed`、`libplayer.so` 或 `libmpv.so` 相关错误 | 确认对应 ABI 的 MPV `.so` 已随 APK 打包；如果改过 JNI/native，先运行 `scripts/build_mpv_player_jni.sh` 后再打包 |
...
- 修改本地 HTTP 服务实现。
- 修改内置资源或 assets。
- 修改 MPV JNI/native 源码时，先运行 `scripts/build_mpv_player_jni.sh` 生成新的 `libplayer.so`，再执行 Gradle 打包。
