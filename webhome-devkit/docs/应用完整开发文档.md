...
只修改 App 侧 WebSocket 连接、解析或设置界面时，不需要重编 Media3 AAR；只有 Media3 API、渲染队列或补丁内容发生变化时才重建并提交新的本地 Maven 产物。
...
当前 `libmpv.so` 已启用 OpenGL、Vulkan、MediaCodec AImageReader/AHardwareBuffer、libcurl 和 HTTP/2。2026-08-10 native 版本矩阵：
...
TV、FFmpeg、mpv-android、media、mpv、libplacebo 和 CatVodSpider 的统一远端 revision 见 `third_party/fongmi-repositories-lock.json`；MPV 整条 native 链的所有 tar 包 SHA-256、子依赖 commit、Meson/Ninja 和 NDK 版本仍以 `third_party/mpv-native-lock.json` 为唯一构建输入。
...
6. WebHTV 在 FongMi 分支上继续应用 `ffmpeg-webhtv-proxy-range.patch`、`mpv-stream-cb-disc-controls.patch` 和 `mpv-matroska-segment-end.patch`。修改光盘控制补丁或 `stream_cb.h` 时必须同时重建 `libmpv.so` 与 `libplayer.so`。
7. FongMi 分支已内建重写后的 AImageReader/AHardwareBuffer OpenGL/Vulkan 后端、异步 fence、HDR/Dolby Vision、双 Surface OSD 和 Android helper scheme；旧 `fd679c81` 与 `mpv-aimagereader-transient-buffer.patch` 不能重复叠加。
8. curl 与 nghttp2 静态链接进 `libmpv.so`，不增加独立网络 `.so`。当前构建不含 HTTP/3、ngtcp2、nghttp3 或 quiche；直接远程 HTTP/HTTPS 可使用 curl，但 App 本地 HLS 代理、`stream_cb` 和 FFmpeg/lavf 路径仍保留。

native 更新后的最低回归范围：

- OpenGL 普通播放和硬解状态。
- OpenGL LUT 生效，预览竖线可见，拖动连续且无闪烁。
- Vulkan 普通播放和 LUT；硬解时确认 `hwdec-current=mediacodec`，并检查 Vulkan AImageReader/YCbCr AHardwareBuffer 日志。
- 电视直出/Dolby Vision 双 Surface 模式下字幕、OSD 和控制层可见，退出或换集不死锁。
