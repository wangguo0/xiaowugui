...
| --- | --- | --- | --- | --- |
| `arm64-v8a` | `0.41.0-556-g9ce79bcaa` | `5ba2525c7aff`（8.0.git） | `7.362.0` / `82224764a981` | OpenGL、Vulkan、LUT 和 Blu-ray ISO 时间轴/seek 已回归 |
...
脚本读取 `third_party/mpv-native-lock.json`，自动应用 MPV 光盘控制补丁；所有下载和中间文件放在被 Git 忽略的 `build/mpv-native/`。Gradle 和 GitHub Actions 始终直接复用仓库已提交的 `.so`，不会增加日常打包时间。完整主机依赖、参数、输出和错误处理见 [`third_party/mpv-native-build.md`](../../third_party/mpv-native-build.md)。
