...
| minSdk | `24` |
| targetSdk | `28` |
| Android NDK | MPV/FFmpeg/libplacebo/JNI 使用 `29.0.14206865`（r29）；IJK/DVD 使用 `28.2.13676358`（r28c）。普通 Gradle 打包直接复用仓库已提交二进制，不要求安装 NDK |
| Media3 | `androidx.media3:*:1.11.0-alpha01-fongmi`；本地 Maven 产物包含上游播放修复、弹幕扩展和 Matroska Dolby Vision RPU 补丁 |
| Exo Dolby Vision | `com.suyashbelekar:exoplayerhdrutils:0.4.0`；基于 libdovi 实时处理 DV7 的 P8.1/HDR10 路径 |
| FFmpeg renderer | `io.github.anilbeesetti:nextlib-media3ext:1.10.0-0.12.1-fongmi-softload-av3a-r1`；内置 FongMi FFmpeg 9 与 libarcdav3a |
...
- JDK 21。JDK 17 会编译失败。
- Git。
- Python 3.10。Chaquo 运行时固定为 3.10，并会在构建时安装 `chaquo/requirements.txt`；只有 Python 3.11/3.12/3.13 不够。
- Android SDK Command-line Tools、Platform 37.0、Build Tools 37.0.0 和 Platform Tools。
- 可访问 Maven Central、Google Maven、Gradle Plugin Portal、JitPack 的网络。

普通 Gradle 打包不编译 MPV、IJK、FFmpeg、OpenSSL、DVD 或 JNI，因而不要求安装 NDK、Perl、`pkg-config` 等 native 工具。只有修改 native 源码、补丁或锁定版本时才安装对应工具链：按 3.5 节使用 NDK `29.0.14206865` 重建 MPV/FFmpeg/libplacebo/JNI，按 3.5.1 节使用 NDK `28.2.13676358` 重建 IJK/DVD。两套 ARM ABI 都不再需要 NDK 21。

macOS 建议先安装 Xcode Command Line Tools，并使用 Homebrew 安装 JDK 21 和 Git：

```bash
xcode-select -p >/dev/null 2>&1 || xcode-select --install
brew install openjdk@21 git python@3.10
export JAVA_HOME="$(brew --prefix openjdk@21)/libexec/openjdk.jdk/Contents/Home"
export PATH="$JAVA_HOME/bin:$(brew --prefix python@3.10)/libexec/bin:$(brew --prefix python@3.10)/bin:$PATH"
java -version
python3.10 --version
```

Ubuntu 24.04 或更新版本可执行：
...
sudo apt-get update
sudo apt-get install -y openjdk-21-jdk git python3.10 python3.10-venv
...
如果使用 Android Studio 自带的 JDK 21，也可以把 `JAVA_HOME` 指向 Android Studio 的 JBR 目录。无论采用哪种安装方式，`java -version` 都必须显示 21。

仓库已内置定制依赖：

- `app/libs/*.aar`：Hook、TVBus、Thunder、ForceTech、JianPian 播放能力依赖。
- `third_party/maven`：定制 Media3 `1.11.0-alpha01-fongmi` 和定制 nextlib。
- `com.suyashbelekar:exoplayerhdrutils:0.4.0`：从 Maven 仓库下载的 Exo DV7/libdovi 转换库，版本由 `gradle/libs.versions.toml` 锁定。
- `third_party/fongmi-repositories-lock.json`：统一记录 TV、FFmpeg、mpv-android、media、mpv、libplacebo 和 CatVodSpider 当前审查/采用的精确 commit。
- `third_party/media-lock.json`：Media3、nextlib、FFmpeg、NDK、CMake 和双 ARM ABI 的精确锁定记录，升级或重编时配合 `scripts/build_media_deps.sh`。
- `third_party/mpv-native-lock.json`：MPV/FFmpeg/libplacebo、构建框架、NDK、工具和全部 native 子依赖的可复现构建输入。
...
- `app/src/main/jniLibs/arm64-v8a`、`app/src/main/jniLibs/armeabi-v7a`：随 APK 打包的 IJK/FFmpeg native 库。
...
确认 JDK、Python 3.10、SDK 命令行工具和 Gradle：
...
`platforms;android-37.0` 是当前 SDK Manager 返回的实际包名；Gradle 中仍写 `compileSdk=37`。如果 `cmdline-tools/latest/bin/sdkmanager` 不存在，请先通过 Android Studio 的 SDK Manager 安装 Android SDK Command-line Tools (latest)。

### 3.3 从零 clone 到打包

完成 3.2 节环境准备后，构建仓库默认分支：
...
Linux 只需把第一行改为 `export ANDROID_HOME="$HOME/Android/Sdk"`。以上命令不会现场重编 native，直接使用仓库已提交的 MPV、IJK、FFmpeg、JNI 和 DVD 库。
...
快速 Release 关闭 minify、资源压缩和 Release lint，版本标识为 `<versionName>-fast-yyyyMMddHHmm`（当前例如 `5.6.0-fast-202608101200`，上海时区）。不传该参数时构建行为不变，仍执行正式 Release 优化，版本标识为 `<versionName>-yyyyMMddHHmm`。快速包仅用于临时验证，不用于正式发布。
...
### 3.4.1 Media3、nextlib/FFmpeg 自编译依赖

普通 App 构建不会现场编译 Media3。仓库已经在 `third_party/maven` 提交 `androidx.media3:*:1.11.0-alpha01-fongmi` 的完整 AAR、sources、POM、Gradle module metadata 和校验文件；`settings.gradle` 优先从该目录解析，`app/build.gradle` 统一锁定版本，CI 和新机器 clone 后可直接使用。

可复现源码信息位于：

- `third_party/media-lock.json`：锁定 FongMi Media3 仓库、分支、提交和发布版本。
- `third_party/patches/media3-*.patch`：本项目在锁定提交上叠加的补丁。
- `third_party/patches/media3-danmaku-live.patch`：实时弹幕批量入口、后台测量、有界 `livePending`、TTL、每帧处理上限、generation 清理和聚合统计，以及对应 Media3 渲染压力测试。
- `third_party/patches/media3-dolby-vision-matroska.patch`：把 Matroska `BlockAdditional` 中的 Dolby Vision RPU 追加到对应 HEVC sample，供 Exo DV7 转换链使用。
- `third_party/patches/nextlib-ffmpeg-soft-load-shedding.patch`：FFmpeg 视频软解负载控制与当前 Android/NDK 构建适配。
- `third_party/patches/nextlib-av3a.patch`：把 nextlib FFmpeg 升级到 FongMi FFmpeg `04482c8d13ac27b2a9fe93f5d388929eef8af5f4`，编译并静态链接 `dependency/avs3a`，映射 `audio/av3a -> libarcdav3a`，支持 10 声道 planar PCM 和必要时立体声下混。
- `scripts/build_media_deps.sh`：检出锁定源码、应用补丁并把模块发布到 `third_party/maven`。

修改 Media3 fork、补丁或锁定版本后执行：

```bash
export ANDROID_HOME="$HOME/Library/Android/sdk"
scripts/build_media_deps.sh
bash gradlew :app:testMobileArm64_v8aDebugUnitTest --tests 'com.fongmi.android.tv.player.danmaku.*'
bash gradlew :app:testMobileArm64_v8aDebugUnitTest --tests 'com.fongmi.android.tv.player.exo.DolbyVisionP81ExtractorsFactoryTest'
bash gradlew :app:assembleMobileArm64_v8aDebug :app:assembleLeanbackArm64_v8aDebug
```

只修改 nextlib、FFmpeg 音频扩展或 AV3A 适配时，不需要重编整套 Media3，可执行：

```bash
export ANDROID_HOME="$HOME/Library/Android/sdk"
scripts/build_media_deps.sh --nextlib-only
bash gradlew :app:assembleMobileArm64_v8aDebug :app:assembleLeanbackArmeabi_v7aDebug
```

nextlib 重建使用 `anilbeesetti/nextlib@6ff6cf9d0820382b3c233d018c52e4163b09d345`、NDK `28.2.13676358`、CMake `3.22.1`，为 `arm64-v8a` 和 `armeabi-v7a` 生成 AAR。脚本会验证两个 ABI 的 `libavcodec.so` 同时包含 `libarcdav3a AV3A` 与 `AV3A Audio Vivid`，然后才写入 `third_party/maven`。普通 App/CI 打包继续直接使用已提交 AAR，不会现场编译 FFmpeg。

Exo AV3A 实机验收使用公开样片 `sample2_ts.zip`。播放参数应显示 `av3a 10ch 48kHz` 和 `ffmpeg...-libarcdav3a`；完整资源地址、校验值和验收结果见 `docs/exo-av3a-acceptance-2026-08-11.md`。

Exo DV7 使用 `ExoplayerHdrUtils 0.4.0` 的 libdovi mode 2 转换链。默认仅在设备不能硬解原始 DV7、但可以硬解 P8.1 时丢弃增强层并改写 RPU；设备可直接硬解 DV7 时保持原码流。P8.1 模式会锁定整次播放，不会在播放中自动切换成 HDR10；只有用户明确选择“降级 HDR10”时才整次使用 HDR10 基底层。`ExoplayerHdrUtils` 本身由 Gradle 下载，修改其调用代码无需重编 Media3；修改 `media3-dolby-vision-matroska.patch` 则必须重新发布本地 Media3 AAR。

WebSocket 弹幕 App 单测使用 MockWebServer 验证握手、突发消息、切源、停止重连和服务端错误重试；该依赖只声明为 `testImplementation`，不会打入 APK。Media3 补丁内的 Robolectric 测试验证 300 条/秒持续输入、300 条瞬时突发、144 条待处理容量、每帧最多 16 次激活、TTL 和清理隔离。发布 GitHub Action 会在四端 release 构建前运行 App 侧完整弹幕测试集。

只修改 App 侧 WebSocket 连接、解析或设置界面时，不需要重编 Media3 AAR；只有 Media3 API、渲染队列或补丁内容发生变化时才重建 Media3。修改 AV3A 解码、FFmpeg 音频 JNI、nextlib renderer 或其锁定版本时必须重建并提交 nextlib AAR。
...
`build_mpv_player_jni.sh` 只重建 `libplayer.so`，而且要求对应 ABI 的 MPV prefix 已由 `scripts/build_mpv_native.sh` 生成。如果修改的是 `libmpv.so`、FFmpeg、libplacebo、MPV commit 或 native lock，应先用 native 脚本完成整套编译、FFmpeg SONAME 重命名、strip 和依赖检查。仅重编相同 client/stream callback API 的 MPV/FFmpeg 时理论上可以复用 `libplayer.so`；修改 `stream_cb.h`、光盘控制补丁、JNI 源码或发布一套新的可复现 lock 时，建议两端一起重建。

当前 `libmpv.so` 已启用 OpenGL、Vulkan、MediaCodec AImageReader/AHardwareBuffer、libcurl 和 HTTP/2。截至 2026-08-17，两套 ABI assets 已按当前 lock、构建脚本和补丁集通过能力标记与 ELF 校验：

| ABI | MPV | FFmpeg | libplacebo | 网络后端 | 状态 |
| --- | --- | --- | --- | --- | --- |
| `arm64-v8a` | `0.41.0-940-gcca559b41` | `04482c8d13ac`（9.0-fongmi） | `7.375.0` / `b694a21bf2dc` | curl 8.21.0 + nghttp2 1.69.0 | 已按 `third_party/mpv-native-lock.json` 重编，并通过能力、ELF 与 APK 资产哈希校验 |
| `armeabi-v7a` | `0.41.0-940-gcca559b41` | `04482c8d13ac`（9.0-fongmi） | `7.375.0` / `b694a21bf2dc` | curl 8.21.0 + nghttp2 1.69.0 | 已使用同一源码 lock 独立重编并通过校验，不能跨 ABI 复制 |

两套 ABI 的源码锁定：

| 组件 | revision |
| --- | --- |
| mpv-android 构建框架/JNI 参考 | `99a60ad2141d5ace94453590903c2c6b9a0a2443` |
| MPV | `cca559b41ceb0bb7731cf6ef2e1f33276cd30c42`（`0.41.0-940-gcca559b41`） |
| FFmpeg | `04482c8d13ac27b2a9fe93f5d388929eef8af5f4`（9.0-fongmi） |
| libplacebo | `b694a21bf2dc176c1e98b8a13c6421a0de5f3da5`（7.375.0 / API 375） |
| curl | 8.21.0，MbedTLS，仅 HTTP/HTTPS，启用 HTTP/2 |
| nghttp2 | 1.69.0 |
| MbedTLS | 3.6.7 |
| libass | `89cc0f4e450d64f74281a17d7f11ed05229665e8` |
| dav1d | `54706fc6bc0cdecab7e9593974a4039cc038fca7`（1.5.4） |
| NDK | `29.0.14206865`（r29），Android API 24 |

TV、FFmpeg、mpv-android、media、mpv、libplacebo 和 CatVodSpider 的统一远端 revision 见 `third_party/fongmi-repositories-lock.json`。MPV 上游仓库、tar 包 SHA-256、子依赖 commit、Meson/Ninja 和 NDK 版本以 `third_party/mpv-native-lock.json` 锁定；最终可复现输入还包括 `scripts/build_mpv_native.sh`、`third_party/patches/`下脚本实际引用的补丁、`third_party/mpv-native-overrides/` 和 JNI 源码。修改任一项都应干净重编相应 ABI，不能只依据 lock 文件判断二进制是否一致。

升级规则：

1. `libmpv.so`、FFmpeg 七个动态库、静态链接进 MPV 的 libplacebo、curl、nghttp2、MbedTLS 和 `libc++_shared.so` 必须按 ABI、按 lock 成套更新。旧 MPV、FFmpeg 8.x 或 libplacebo API 371 不再是正式组合。
2. 当前 MPV 直接依赖 FFmpeg 9 的 HDR、Dolby Vision、MMT/TTML、MediaCodec API 和 libplacebo API 375。不要通过篡改 pkg-config 版本号强行链接旧 libplacebo，也不要从 FFmpeg 9 分支零散摘取单个功能提交。
...
6. FFmpeg 继续应用 `ffmpeg-webhtv-proxy-range.patch` 和 `ffmpeg-mediacodec-port-starvation.patch`，分别处理 App 内部 Range 起点与 MediaCodec 端口长时饥饿时的有界失败/回退。
7. MPV 播放语义补丁包括 `mpv-stream-cb-disc-controls.patch`、`mpv-android-dovi-el-surface.patch`、`mpv-dovi-profile7-hdr10-base-layer.patch`、`mpv-audiotrack-truehd-channel-mask.patch` 和 `mpv-matroska-segment-end.patch`。修改光盘控制补丁、`stream_cb.h` 或 JNI 源码时必须同时重建 `libmpv.so` 与 `libplayer.so`。
8. Android 输出补丁包括 `mpv-mediacodec-embed-optional-osd.patch`、`mpv-mediacodec-embed-timed-release.patch`、`mpv-mediacodec-output-timing-diagnostics.patch`、`mpv-android-vulkan-conversion-default.patch`、`mpv-android-vulkan-smart-backend.patch`、`mpv-android-vulkan-legacy-backend.patch` 和 `mpv-aimagereader-stable-flow.patch`。它们共同定义可选 OSD Surface、按 PTS 提交 MediaCodec 输出、掉帧时序诊断及 Vulkan `auto/direct/legacy/stable/compute/fragment` 后端。
9. FongMi 分支已内建重写后的 AImageReader/AHardwareBuffer OpenGL/Vulkan 后端、异步 fence、HDR/Dolby Vision、双 Surface OSD 和 Android helper scheme；旧 `fd679c81` 与 `mpv-aimagereader-transient-buffer.patch` 不能重复叠加。
10. curl 与 nghttp2 静态链接进 `libmpv.so`，不增加独立网络 `.so`。当前构建不含 HTTP/3、ngtcp2、nghttp3 或 quiche；直接远程 HTTP/HTTPS 可使用 curl，但 App 本地 HLS 代理、`stream_cb` 和 FFmpeg/lavf 路径仍保留。

native 更新后的最低回归范围：

- OpenGL 普通播放和硬解状态。
- OpenGL LUT 生效，预览竖线可见，拖动连续且无闪烁。
- Vulkan 普通播放和 LUT；硬解时确认 `hwdec-current=mediacodec`，并分别验证 `auto/direct/legacy/stable` 的选择和回退日志。
- 电视直出/Dolby Vision 在有 OSD Surface 时字幕和控制层可见；关闭字幕时允许不创建额外 OSD Surface，退出、换集和内核切换不得死锁。
- MediaCodec 直出检查 timestamped release 和输出时序诊断，不得因立即释放缓冲帧导致持续掉帧，也不得引入新的长时间调度阻塞。
- Dolby Vision Profile 7 分别验证原码流直出与 HDR10 基底层模式，播放中不得自动在 DV/HDR10 之间反复切换。
- MMT/TLV、TTML/ARIB、AV3A、压缩包和直播状态。
- 文本字幕/图形字幕加载及播放中切换。
- 进入播放失败前切换播放器内核。
...
历史兼容性结论：旧 MPV 与 FFmpeg 8.1.x 的若干重编组合曾在 vivo Android 15 播放初始化阶段触发 destroyed-mutex native 崩溃；该结果只说明旧组合不应混用，不能归因于某一个单独源文件。当前正式 lock 已整体升级到 FongMi FFmpeg 9、MPV 940、libplacebo API 375 和 NDK r29，验证必须以当前整套资产为单位。
...
# 按需执行；发布一套新 lock 时建议执行：
scripts/build_mpv_player_jni.sh --abi all --install
```

脚本读取 `third_party/mpv-native-lock.json`，下载固定源码和 tar 包，再按脚本中声明的顺序应用 FFmpeg Range/MediaCodec 回退、MPV 光盘/Dolby Vision/AudioTrack、直出 OSD/timed release/时序诊断、Vulkan 后端和 Matroska seek 补丁，然后构建 FFmpeg 9、字幕/字体/光盘/归档/网络/Vulkan 依赖，修改 FFmpeg ELF 名称，strip 并校验能力与 `DT_NEEDED`。所有下载和中间文件默认放在被 Git 忽略的 `build/mpv-native/`；可用 `--work-dir` 指向独立缓存目录。Gradle 和 GitHub Actions 始终复用仓库已提交的 `.so`，不会现场重编 MPV；Android Release Action 会先运行 `scripts/verify_mpv_native_assets.sh --require-elf`。
...
| OpenSSL | `openssl-3.2` |
| arm64 NDK | `28.2.13676358`，与 DVD 共用；MPV/JNI 使用 r29 |
| armv7a NDK | `28.2.13676358`，与 DVD 共用；MPV/JNI 使用 r29 |

arm64-v8a 和 armeabi-v7a 的 IJK 均已在 macOS 使用 NDK 28.2.13676358 完整重建成功，并通过 ELF、`SONAME`、`DT_NEEDED` 和未解析标准流符号检查。NDK 28 可以编译 32 位 ARM，旧的 NDK 21.4.7075529 已不再需要。该版本只属于 IJK/DVD lock，不要拿它重建当前 MPV/JNI。
...
Xcode Command Line Tools 提供 `make`，macOS 系统自带的 Perl 和 `file` 可以直接使用。两套 ARM ABI 都不要求 `yasm`；它只用于本脚本目前不支持的 x86/x86_64 汇编路径。

Ubuntu 24.04 或更新版本安装 native 工具：
...
从零 clone 后依次重建两套 ABI、复制产物并打包四个快速 Release APK：
...
Ubuntu 使用同一组命令，只需把 `ANDROID_HOME` 改为 `$HOME/Android/Sdk`。只更新一个 ABI 时，可以只执行对应的 `build_ijk_native.sh --abi ... --install` 和 Gradle flavor；不要把某个 ABI 的三项 `.so` 复制给另一个 ABI。

脚本行为：

1. 将源码下载到 `third_party/sources/ijkplayer-android`，并强制切换到锁定 revision。
2. 应用仓库锁定补丁，并使用 FFmpeg 4.0 与 OpenSSL `openssl-3.2` 编译 OpenSSL、FFmpeg 静态库和 IJK JNI。
...
OpenSSL、FFmpeg 和 IJK native 统一以 Android API 21 编译。补丁会保留 `-D__ANDROID_API__=21`，在 NDK 28 下停用旧版 armv7a 的 `-nostartfiles`，并把 `PKG_CONFIG_LIBDIR` 限制到本次生成的目标端 OpenSSL，同时清空 `PKG_CONFIG_PATH`。这样可避免 Homebrew 或 Linux 主机上的 OpenSSL 被误链接，也避免 OpenSSL 按 API 35 编译、FFmpeg 按 API 21 链接时出现 `stdin`/`stdout`/`stderr` 未解析符号。
...
当前 Exo、MPV 的输出路径不能仅根据设置界面的渲染选项判断：

- Exo 未使用 LUT 时遵循用户渲染设置，默认为 `SurfaceView`，保留 MediaCodec 直接输出的性能优势。
- Exo 启用且可用 LUT 时，运行时临时使用 `TextureView`。LUT 通过 Media3 `VideoGraph`/GL 处理，该切换只改变最终输出 View，不会把 MediaCodec 硬解改成软解；关闭 LUT 后恢复用户原渲染设置。这是为规避部分横屏设备在 Media3 Effects + 裁剪 `SurfaceView` 组合下的 BLAST buffer 尺寸/旋转拒绝问题。
- MPV 不使用上述 Exo `TextureView` 兼容路径。电视直出使用 MediaCodec 视频 Surface，字幕/OSD 需要时使用独立透明 OSD Surface；OpenGL/Vulkan/LUT 使用 MPV/libplacebo 自身的 GPU 渲染链。

Exo DV7 的实际选择规则：

1. 设备可直接硬解原始 DV7 时保持原码流。
2. 原始 DV7 不支持、但 P8.1 可硬解且用户选择“升级 P8.1”时，使用 `ExoplayerHdrUtils`/libdovi mode 2 改写 RPU 并丢弃 EL。
3. 用户选择“降级 HDR10”时，整次播放仅使用 HDR10 基底层。
4. 输出模式在一次播放中锁定，不允许根据解码器短暂信号自动在 P8.1 和 HDR10 之间反复切换。

构建边界：修改 App 侧 DV7 策略、渲染选择或 LUT 界面只需重新打包 App；修改 Media3 extractor 的 Matroska RPU 补丁要重建本地 Media3 AAR；修改 MPV/FFmpeg/Vulkan/AImageReader 补丁要用 r29 重建对应 ABI 的整套 native assets；修改 JNI 源码或 client API 还要同步重建 `libplayer.so`。
...
```text
app/build/outputs/apk/mobileArm64_v8a/debug/shouji-arm64_v8a-debug.apk
app/build/outputs/apk/mobileArmeabi_v7a/debug/shouji-armeabi_v7a-debug.apk
app/build/outputs/apk/leanbackArm64_v8a/debug/dianshi-arm64_v8a-debug.apk
app/build/outputs/apk/leanbackArmeabi_v7a/debug/dianshi-armeabi_v7a-debug.apk
```
...
Release/apk/shouji-armeabi_v7a.apk
Release/apk/dianshi-arm64_v8a.apk
Release/apk/dianshi-armeabi_v7a.apk
```
...
```bash
adb devices
adb install -r app/build/outputs/apk/mobileArm64_v8a/debug/shouji-arm64_v8a-debug.apk
```
...
| `Chaquopy ... is not a valid Python 3.10 command` 或找不到 `python3.10` | 安装主机 Python 3.10，并确保 `python3.10 --version` 可在当前终端执行；仅安装系统默认 Python 3.12 不够 |
| `SDK location not found` | 缺少 `local.properties`，或 `sdk.dir` 指向错误 |
| `.../cmdline-tools/latest/bin/sdkmanager: No such file or directory` | 通过 Android Studio SDK Manager 安装 Android SDK Command-line Tools (latest)，并检查 `ANDROID_HOME` |
| `failed to find target with hash string 'android-37'` | 安装 Android SDK Platform 37 |
| `NDK clang++ not found` | 重建 MPV/JNI 时安装 `29.0.14206865`；重建 IJK/DVD 时安装 `28.2.13676358`，并让 `ANDROID_NDK_HOME` 与对应 lock 一致 |
| `Missing tool: perl` | macOS 检查系统 Perl；Ubuntu 安装 `perl` |
| `Missing tool: pkg-config` | macOS 执行 `brew install pkg-config`；Ubuntu 安装 `pkg-config` |
| IJK 链接时报未解析的 `stdin`、`stdout` 或 `stderr` | OpenSSL 与 FFmpeg 的 Android API 不一致，或残留旧构建；确认使用当前补丁和 NDK 28，然后加 `--clean` 重新运行 `bash scripts/build_ijk_native.sh` |
| IJK 日志引用 `/usr/local/opt/openssl`、`/opt/homebrew/opt/openssl` 或主机 OpenSSL | 使用当前脚本并加 `--clean` 重建；脚本会隔离目标端 `pkg-config`，不要手工注入主机 `PKG_CONFIG_PATH` |
...
| `missing llvm-readelf/readelf` | Linux 安装 `binutils`；macOS 校验 MPV 时安装 NDK `29.0.14206865`，校验 IJK/DVD 时使用对应 r28c，或设置 `ANDROID_NDK_HOME`/`READELF` |
...
仓库内置的五种远程托管服务端都实现了同一套观影同步协议，可按部署平台选择：

| 版本 | 观影记录存储 | 说明 |
| --- | --- | --- |
| [Cloudflare Worker](../../serverless/webhtv-remote-cloudflare/README.md) | 独立 `PLAYBACK_DO` + SQLite | 与 relay 合并域名、状态隔离，适合 Cloudflare 部署 |
| [Deno Deploy](../../serverless/webhtv-remote-deno/README.md) | Deno KV 分块状态 | 使用 versionstamp 原子更新 |
| [Vercel Edge](../../serverless/webhtv-remote-vercel/README.md) | Vercel KV / Upstash Redis REST | 必须配置 Redis 环境变量；未配置时接口返回 503 |
| [Go](../../serverless/webhtv-remote-go/README.md) | 本地原子 JSON 文件 | 适合单实例 VPS、NAS、容器 |
| [Rust](../../serverless/webhtv-remote-rust/README.md) | 本地原子 JSON 文件 | 适合单实例 VPS、NAS、容器 |
...
该地址通过 `POST` 接收进度、完播和删除 Webhook，通过 `GET` 返回 `{ changes, nextSince, hasMore }` 增量结果。内置服务端强制要求 `X-WebHTV-Token` 和 `X-WebHTV-Config-Key`；多台设备需要使用同一 token，不同用户或不同数据空间应使用不同 token。Webhook 字段预设应选“基础”“标准”或“完整”，匿名预设以及缺少核心影片字段的自定义预设不能形成完整的同步记录。各版本都保留 90 天删除墓碑，且只有显式 `scope=all` 才允许生成全量删除。具体部署、存储和备份边界见对应目录 README。
...
```json
{
  ...
  "deleted": [
    {
      "configKey": "sha256(config-url)",
      "historyKey": "site_key@@@deleted_vod_id@@@1",
      "siteKey": "site_key",
...
删除项可放在 `deleted`、`deletions`、`tombstones`、`removed` 或 `deletedItems` 数组中；也可与普通记录一起放在 `items`/`changes` 中，并使用 `action: "delete"`、`deleted: true` 或 `event: "playback.deleted"` 标识。删除定位规则与本机清理 API 相同：

- `historyKey` 或 `siteKey + vodId`：删除单条影片记录，`scope` 可省略或写 `item`。
- `siteKey + scope=site`：删除该站点记录。
- `scope=all`：删除当前 `configKey` 下的记录。
- `deletedAt` 必须使用删除发生时的毫秒时间戳。App 会保留 90 天删除墓碑；旧于墓碑的进度会被跳过，新于墓碑的进度可以重新创建记录。

服务端返回 `nextSince` 后，App 会按 `configKey` 保存游标，并在下一次请求通过 `X-WebHTV-Since` 发送。若响应条数超过 App 的 `maxItems`，App 不推进游标，避免未处理变更永久丢失。服务端应尽量遵守 `X-WebHTV-Limit`，并在增量结果中继续返回尚未过期的删除墓碑。
...
服务端收到删除事件后应保存墓碑，而不只是物理删除当前行；否则离线设备稍后上传的旧进度可能再次创建该记录。建议墓碑保留时间不少于 App 的 90 天，并按 `token + configKey + scope + historyKey/siteKey/vodId` 去重，以较新的 `deletedAt` 为准。
