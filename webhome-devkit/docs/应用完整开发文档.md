...
| `armeabi-v7a` | `0.41.0-878-g94335ab87` | `894da5ca7d74`（n8.0.1） | `7.371.0` / `a7a18af88ff0` | 同一 lock 独立构建并通过版本、补丁及 ELF 校验；待 32 位真机播放回归 |
...
1. `libmpv.so`、FFmpeg 七个动态库、静态链接进 MPV 的 libplacebo 和 `libc++_shared.so` 必须按 ABI、按 lock 成套更新。此前“旧 `libmpv.so` + FFmpeg 8.1.2”只用于止崩隔离，不再是正式组合。
...
已知兼容性限制：MPV `94335ab87` 或旧 MPV 与 FFmpeg 8.1.2 重新构建的组合，会在 vivo Android 15 真机播放初始化阶段触发 destroyed-mutex native 崩溃；改用上游 mpv-android 同期采用的 FFmpeg n8.0.1 后，最新 MPV 在同一设备、同一播放场景正常。现有证据锁定的是“FFmpeg 8.1.x 构建组合兼容问题”，不能仅凭该结果断言 FFmpeg 的某一个源文件是唯一根因。
...
脚本读取 `third_party/mpv-native-lock.json`，自动应用 MPV 光盘控制补丁；所有下载和中间文件放在被 Git 忽略的 `build/mpv-native/`。当前 lock 与两套已提交 assets 一致，可复现正式组合。Gradle 和 GitHub Actions 始终复用仓库已提交的 `.so`。完整排查记录见本地 `plans/MPV原生依赖升级与Android崩溃排查记录.md`。
