...
`build_mpv_player_jni.sh` 只重建 `libplayer.so`。如果修改的是 `app/src/*/assets/mpv-libs/*` 下的 `libmpv.so`、FFmpeg 或 libplacebo，需要在外部完成整套 native 编译、重命名、strip 和依赖检查；只要 MPV client API 没变，通常不必重建 `libplayer.so`。

当前 native 版本矩阵：

| ABI | MPV | FFmpeg | libplacebo | 状态 |
| --- | --- | --- | --- | --- |
| `arm64-v8a` | `0.41.0-556-g9ce79bcaa` | `5ba2525c7aff`（8.0.git） | `7.362.0` / `82224764a981` | OpenGL、Vulkan、LUT 预览分割线已回归 |
| `armeabi-v7a` | `0.41.0-224-gd54bad563` | `N-122998-g5ba2525c7a` | `7.360.0-3-gc93aa134` | 原 32 位 Vulkan 基线 |
...
1. `libmpv.so`、FFmpeg 七个动态库和 `libc++_shared.so` 必须按 ABI 成套更新。MPV 与 FFmpeg 有直接符号依赖，只替换 `libmpv.so` 会出现类似 `cannot locate symbol "av_dynamic_hdr_*"` 的加载错误。
2. MPV `9ce79bcaa` 要求 libplacebo `>= 7.360.1`；当前使用 `82224764`（项目版本 `7.362.0`）。不要通过篡改 pkg-config 版本号强行链接旧 libplacebo。
...
5. arm64 和 armeabi-v7a 是独立构建产物，不能跨 ABI 复制；升级一个 ABI 不代表另一个 ABI 已同步。
...
脚本读取 `third_party/mpv-native-lock.json`，所有下载和中间文件放在被 Git 忽略的 `build/mpv-native/`。Gradle 和 GitHub Actions 始终直接复用仓库已提交的 `.so`，不会增加日常打包时间。完整主机依赖、参数、输出和错误处理见 [`third_party/mpv-native-build.md`](../../third_party/mpv-native-build.md)。
