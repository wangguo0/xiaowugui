...
10. 使用 NDK `llvm-readelf` 检查每个 SONAME、MPV 的完整依赖和 Vulkan 依赖，并检查 MPV/libplacebo/curl 版本字符串、HTTP/2标记、libass fontconfig 字符串、光盘控制、AImageReader暂态、代理Range及Matroska Segment补丁标识；同时拒绝动态 `libfontconfig.so` 或 `libexpat.so` 依赖。

`scripts/verify_mpv_native_assets.sh` 对已提交 assets 执行同类校验，Android Release Action 会在 Gradle 打包四个 APK 前以 `--require-elf` 模式调用它，并确认AImageReader暂态、代理Range与Matroska Segment修复已进入两套原生资产，防止 lock、补丁、arm64/armv7 assets 或静态网络能力不一致的二进制进入 Release。
