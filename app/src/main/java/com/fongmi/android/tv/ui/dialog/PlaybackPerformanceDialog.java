...
public final class PlaybackPerformanceDialog extends DialogFragment {
    ...
    private void showHelpDialog() {
        ...
        addHelpIntro(content, "当前播放器内核：" + playerName() + "。不知道怎么选时保持“自动”（默认）；每项说明都会明确告诉你什么情况更流畅、异常时改哪一档，以及对应代价。EXO会按当前协议、分片和链路动态锁定起播/重缓冲门槛并协调预载；MPV会在符合条件的电视4K场景自动使用低开销电视直出；IJK自动档采用稳定基线。多数底层参数需要重新进入播放或重建播放器后生效。");
        String section = "";
