...
public final class PlaybackPerformanceCatalog {
    ...
    private static void addExo(List<PlaybackPerformanceOption> options) {
        ...
        options.add(option(EXO_NETWORK_PROTECTION, BUFFER, "网络保护降速", "作用：网盘单轨视频的持续吞吐略低于消费码率时，根据5～30秒缓冲趋势在1.00x与保护下限之间每次0.02x渐进调节，并用PCM Sonic保调降低单位时间的数据消耗。低感知最低0.95x，增强最低0.90x，激进最低0.85x；仅EXO点播且用户倍速为1.0时生效。达到下限后仍持续掉缓冲会报告带宽不足，不会无限降速。代价：会关闭隧道、音频直通和刷新率强制匹配；严重带宽不足仍无法稳定。修改后重新进入播放生效。"));
    ...
    private static String profileDescription(int kernel) {
        return switch (kernel) {
            ...
            default -> "首选“自动”（也是默认）：以均衡参数起步，根据真实缓冲、视频码率和带宽把预载调为0～2线程，并在当前播放中把重缓冲恢复逐级调到2～15秒。“均衡”固定为1.5秒起播/3秒恢复；“兼容”用同步队列、2秒起播/5秒恢复，适合异步解码异常但启动更慢；“轻量”缩小缓存、1秒起播/2秒恢复，省内存但抗波动更弱。手动改任一项后显示“自定义”。";
        };
    }
