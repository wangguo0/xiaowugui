...
public final class PlaybackPerformanceCatalog {
    ...
    private static void addExo(List<PlaybackPerformanceOption> options) {
        ...
        options.add(option(EXO_NETWORK_PROTECTION, BUFFER, "无感网络保护", "作用：仅在EXO点播、用户1.00x、当前未启用隧道和音频直通时，持续判断缓冲斜率、耗尽时间与重缓冲风险；确认轻微且可在0.97～1.00x内补偿的吞吐缺口后才自动介入，恢复后自动回到1.00x并退出。设置开启只代表允许判断，不会关闭隧道、音频直通，也不会改变电视帧率/分辨率直出。若缺口超过无感范围则保持休眠，不会无限降速。播放性能“自动”默认允许此判断。"));
