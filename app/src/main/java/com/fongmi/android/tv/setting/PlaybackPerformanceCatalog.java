...
public final class PlaybackPerformanceCatalog {
    ...
    private static void addExo(List<PlaybackPerformanceOption> options) {
        ...
        options.add(option(EXO_REBUFFER, BUFFER, "重缓冲恢复", "作用：卡住后积累多少缓冲才恢复。自动档会按协议、分片边界、可信吞吐、time-to-empty和重缓冲历史为每次恢复锁定1～15秒门槛；手动建议均衡3秒、兼容5秒、轻量2秒。代价：数值越高越不易再次卡，但单次等待更久。"));
        ...
        options.add(option(EXO_NETWORK_PROTECTION, BUFFER, "动态网络保护", "作用：仅在EXO点播、用户1.00x、当前未启用隧道和音频直通时，根据缓冲快慢趋势、安全缓冲线、剩余救援时间和可信吞吐连续计算目标倍率。优先使用0.97～1.00x轻量保护；只有持续缺口确实需要且可在缓冲耗尽前稳定时，才按计算目标进入0.85～0.97x，不会异常后直接跳到0.85x。恢复后自动回到1.00x。不会关闭隧道、音频直通或电视直出；低于0.85x仍无法稳定时会报告保护能力不足。"));
    ...
    private static String profileDescription(int kernel) {
        return switch (kernel) {
            ...
            default -> "首选“自动”（也是默认）：以均衡参数起步，根据协议、分片、可信吞吐和缓冲趋势动态锁定每次起播/重缓冲门槛，并根据前台余量控制预载。“均衡”固定为1.5秒起播/3秒恢复；“兼容”用同步队列、2秒起播/5秒恢复，适合异步解码异常但启动更慢；“轻量”缩小缓存、1秒起播/2秒恢复，省内存但抗波动更弱。手动改任一项后显示“自定义”。";
        };
    }
