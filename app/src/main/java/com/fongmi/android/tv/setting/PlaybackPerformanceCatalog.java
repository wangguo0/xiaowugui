...
public final class PlaybackPerformanceCatalog {
    ...
    private static void addExo(List<PlaybackPerformanceOption> options) {
        ...
        options.add(option(DV7_HDR10_FALLBACK, DECODE, "DV7处理", "默认“升级P8.1”：设备不支持当前DV7硬解、但支持P8.1硬解时，使用libdovi mode 2实时改写RPU并丢弃增强层；原生DV7可硬解时保持原样。选择“降级HDR10”会直接使用基底层，兼容性更高但失去Dolby Vision动态元数据。P8.1转换不可用时仍会自动回退HDR10，避免黑屏。"));
