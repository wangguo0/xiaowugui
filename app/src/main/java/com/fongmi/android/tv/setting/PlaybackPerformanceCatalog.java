...
public final class PlaybackPerformanceCatalog {
    ...
    private static void addExo(List<PlaybackPerformanceOption> options) {
        ...
        options.add(option(EXO_REBUFFER, BUFFER, "重缓冲恢复", "作用：卡住后积累多少缓冲才恢复。自动档会按协议、分片边界、可信吞吐、time-to-empty和重缓冲历史为每次恢复锁定1～15秒门槛；历史只在相同网络、路径、协议和资源类型中复用，并会自动过期。手动建议均衡3秒、兼容5秒、轻量2秒。代价：数值越高越不易再次卡，但单次等待更久。"));
        ...
        options.add(option(EXO_NETWORK_PROTECTION, BUFFER, "单码率网络救援", "作用：自动档默认关闭，只有你单独确认后，EXO才会在明确只有一个视频码率、点播、用户1.00x、非隧道和非音频直通时临时降到0.97～1.00x。多码率HLS/DASH始终交给Media3 ABR降轨，资源证据未知时不会启动。每次最多60秒、每个媒体最多2次，两次之间至少冷却60秒；Seek、暂停、切资源、手动改速、关闭实验或退出都会立即恢复1.00x。代价：救援期间片长会暂时延长，极端弱网可能仍会重缓冲。"));
    ...
    private static String profileDescription(
        ...
        if (recommendedMerged) {
            return switch (kernel) {
                case PlayerSetting.MPV -> "首选“自动”：电视4K硬解且不需要MPV字幕/LUT/shader/滤镜时自动用低开销电视直出；HLS按可信吞吐保守起步，持续网络风险时逐档重载降码率。“兼容”改用GPU完整＋mediacodec-copy，适合零拷贝异常但4K可能更卡；“轻量”限制HLS至8Mbps并降低缓存，适合低内存/低性能设备。旧均衡档已合并到自动。手动改任一项后显示“自定义”。";
                case PlayerSetting.IJK -> "首选“自动”：按协议采用稳定的点播/直播基线并根据运行反馈有界调整。“兼容”提高水位、探测和画面队列，起播/恢复更慢但更稳；“轻量”降到4MB、快速探测和积极丢帧，省内存但更容易卡顿和损失画面。旧均衡档已合并到自动。手动改任一项后显示“自定义”。";
                default -> "首选“自动”（也是默认）：以保守参数起步，根据协议、分片、可信吞吐和缓冲趋势动态锁定起播/重缓冲门槛；历史按网络和资源隔离并自动过期，同时根据前台余量控制预载。“兼容”使用同步队列和更保守恢复，适合异步解码异常但启动更慢；“轻量”缩小缓存，省内存但抗波动更弱。旧均衡档已合并到自动。手动改任一项后显示“自定义”。";
            };
        }
        return switch (kernel) {
