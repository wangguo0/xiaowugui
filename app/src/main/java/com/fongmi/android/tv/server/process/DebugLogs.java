...
public class DebugLogs implements Process {
    ...
    private String css() {
        return "html,body{box-sizing:border-box;width:100%;max-width:100%;margin:0;overflow-x:hidden;background:#f4f6f8;color:#1f2328;font:14px/1.5 -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;}*,*:before,*:after{box-sizing:inherit;min-width:0}body{position:relative}"
                + "header{box-sizing:border-box;position:sticky;top:0;z-index:3;display:flex;flex-wrap:wrap;gap:8px;align-items:center;width:100%;max-width:100%;padding:8px 10px;background:rgba(255,255,255,.96);border-bottom:1px solid #d8dee4;box-shadow:0 2px 10px rgba(31,35,40,.06);overflow:hidden}"
                ...
                + "@media(max-width:680px){header{padding:7px 8px}h1{font-size:16px}.meta{margin-left:4px}.addr{grid-template-columns:1fr}.tools{top:38px}.search{grid-template-columns:minmax(0,1fr) auto auto}.title{white-space:normal}.time{display:none}.entry{padding:8px}.detail{font-size:13px}}";
    }
    ...
    private String scriptEnhanced() {
        return "const rawEl=document.getElementById('raw'),logs=document.getElementById('logs'),meta=document.getElementById('meta'),summary=document.getElementById('summary'),filter=document.getElementById('filter'),simple=document.getElementById('simple'),pause=document.getElementById('pause'),download=document.getElementById('download');"
                ...
                + "function resetX(){try{document.documentElement.scrollLeft=0;document.body.scrollLeft=0;scrollTo(0,scrollY||pageYOffset||0)}catch(e){}}"
