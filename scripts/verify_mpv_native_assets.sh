...
verify_abi() {
  ...
  contains_string "$directory/libmpv.so" "DV7 HDR10 fallback: preserving decoder input and stripping Dolby Vision frame metadata."
  reject_string "$directory/libmpv.so" "DV7 HDR10 fallback: stripping EL/RPU before decoder."
  reject_string "$directory/libmpv.so" "DV7 HDR10 fallback: failed to produce base-layer packet."
